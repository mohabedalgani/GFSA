...
def genpop(pop, params, runNumber):
    ...
    for i in range(params['maxGen']):
        ...
        for value in bestfit['chromNum']:
            ...
            if lv != '0' and rv != '0':
                fold += 1
        folds.append(fold)
        f4_plot.plot(maximum_fit, color='b', marker='.') # maximum fit
        f4_plot.plot(folds, color='r', marker='.')
        plt.pause(0.01)
    ...
    if params['type'] == 'binaryGF': # no roc
        if len(np.unique(testY)) > 1:
            y_pred_proba = bestmodel.predict_proba(testX)[:, 1]
            #y_pred_proba2  = 0.5 <= y_pred_proba
            y_true = (y_pred_proba <= 0.5)
            fpr, tpr, _ = roc_curve(testY, y_pred_proba , pos_label=int)
